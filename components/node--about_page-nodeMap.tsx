...
// const view = await drupal.getView("team--block")
...
export function NodeAboutPage({ node, ...props }: NodePageProps) {
    ...
    useEffect(() => {
        async function fetchNodes() {
          if (node.main_entity) {
            const fetchedNodes = await Promise.all(node.main_entity.map(async item => {
              const fetchedNode = await drupal.getResource(item.type, item.id);
              return fetchedNode;
            }));
